...
export class EditEventComponent implements OnInit {
  ...
  updateEvent(user: NgForm):void {
    ...
    const body = { eventid: user.value.eventid, name: user.value.name, type: user.value.type, playercount: user.value.playercount, date: user.value.date };
    this.http.put<any>('https://kw31bx8r49.execute-api.us-east-1.amazonaws.com/events/update', body, { headers })
            .subscribe(data => {
              if(data == '200') {
                this.dialog.closeAll();
                this.snackBar.open("Successfully Updated Event", "Okay", {
                ...
                this.dialog.closeAll();
                this.snackBar.open("Could Not Update Event", "Okay", {
                  duration: this.durationInSeconds * 1000,
